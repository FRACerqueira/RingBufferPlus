...
| **U-10** | `LockWhenScaling()` é um método público, visível no IntelliSense, no builder de autoscale, que é um no-op documentado — ver §4.1. **Pendente** — depende da Decisão A em §4 (remover vs. manter com `[Obsolete]`), ainda não tomada. A evidência concreta citada aqui (o sample RabbitMQ enganado por isso) foi corrigida junto com U-06 — o método em si ainda existe, sem mudança. | `Commands/IRingBufferAutoScaleBuilder.cs:65-73` |
...
**Sequência seguida:** P0 e a quase totalidade do P1 implementados com protocolo vermelho/verde (ver [`plano-de-acao.md`](./plano-de-acao.md) para o registro completo). A Decisão A (§4.1) virou [ADR007 V02](../doc/adr/ADR007V02-redesign-of-the-public-fluent-api-surface.md) + [ADR004 V02](../doc/adr/ADR004V02-semantic-versioning-policy-and-fluent-api-stability.md) (exceção de versionamento) e já foi implementada. Restam: U-07 e as Decisões B/C (mesma questão, código vs. doc no limiar de falhas — decidido corrigir o código) e a implementação da Decisão B (caminho de retry para warmup).
