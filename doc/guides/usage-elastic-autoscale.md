...
[**Back to README**](../../README.md) · See also: [Concepts](concepts.md) · [ADR003](../adr/ADR003V02-median-sample-autoscaling-algorithm.md)
...
`AutoScaleAcquireFault(numberOfFaults)` returns an `IRingBufferAutoScaleBuilder<T>`. `Build`/`BuildWarmupAsync` from that type return a plain `IRingBufferService<T>` — `SwitchToAsync` is not part of its surface, by design ([ADR007](../adr/ADR007V02-redesign-of-the-public-fluent-api-surface.md)).

## What happens internally

- **Scale up:** every timed-out `AcquireAsync` increments a fault counter in the engine loop. Once the counter reaches `numberOfFaults`, the buffer moves to `Capacity` if it is currently below `Capacity` (a partial recovery step - this also covers a fresh buffer where `initialCapacity == minCapacity`), or straight to `MaxCapacity` otherwise, and the counter resets. Because faults are counted per timed-out *acquire call*, not per internal retry as in v4, reaction time is roughly `AcquireTimeout × numberOfFaults` — lower `AcquireTimeout` if you need faster reaction (see the `CHANGELOG.md` "Breaking changes v5.0.0" entry on this behavior change).
- **Scale down:** on every sampling tick (`baseTimer` / `numberSamples` — e.g. above, one sample roughly every 100ms, evaluated as a median every 5 seconds), the engine computes the median of collected samples via `AutoScaleDecision.Median` and decides whether to step back down toward `MinCapacity`/`Capacity` via `AutoScaleDecision.EvaluateScaleDown` — see [ADR003](../adr/ADR003V02-median-sample-autoscaling-algorithm.md) for the exact thresholds and why the algorithm is a pure, independently-tested function.
- Scale-up has a deadline of `quantity × FactoryTimeout` (the factory's own per-item timeout, set via `Factory(value, timeout)`); scale-down never waits for more items to become idle - it only takes whatever is already idle right now. It can still briefly wait once it has decided which items to remove, though: disposing each one is bounded by `PulseHeartBeat` (configurable via `HeartBeat(callback, pulse: ...)`'s `pulse` parameter, defaulting to 10 seconds even if `HeartBeat` itself is never configured - see [the heartbeat guide](usage-heartbeat.md)) - a hung `Dispose()`/`DisposeAsync()` on a removed item is logged and left to finish in the background rather than blocking the scale-down forever. Neither direction "undoes" a partial result: a scale-up that only creates some of the requested items keeps them, and a scale-down that only finds some items idle removes just those - see [ADR001](../adr/ADR001V02-concurrency-model-for-ring-buffer-manager-scale-up-and-down.md).
...
- `maxConsecutiveFactoryFailures` (set via `Factory(value, timeout, maxConsecutiveFactoryFailures)`) combines multiplicatively with the trade-off above when the tolerated failures are themselves timeouts, not fast exceptions: tolerating `N` consecutive timeouts before giving up on an item raises that item's own worst-case blocking time to roughly `(N + 1) × FactoryTimeout`, on top of `quantity × FactoryTimeout` for the whole batch. This is a documentation note, not a code change — the existing per-scale-up-batch deadline (`quantity × FactoryTimeout`, see above) is still the outer bound regardless.
- Honoring the `CancellationToken` your `Factory` delegate receives is your responsibility, not this library's: the token now fires at the per-item `FactoryTimeout` deadline (and on shutdown), but a factory that ignores it and keeps working anyway just keeps running, orphaned, after the engine has already given up waiting on it. If it eventually succeeds, the instance it produces is discarded without being disposed — a leaked database connection, broker channel, or similar, that this library cannot detect or clean up on your behalf. Pass the token through to any awaited I/O inside your factory to avoid this.
- Picking `initialCapacity: 2` or `minCapacity: 2` (the minimum legal value for either) used to have a side effect on scale-down — both come from the same root cause, the scale-down safety margin reducing to the buffer's own current capacity when the paired boundary value is 2 — but both are now fixed (Round 4, findings R18/R19): the threshold is capped so it can never reach current capacity itself. With `initialCapacity: 2`, scale-down from above initial capacity now still fires, just requiring a very high (not literally impossible) idleness. With `minCapacity: 2`, scale-down to the floor no longer requires exactly zero acquisitions in the sampling window — one active item is now tolerated, same as the cap allows at any other capacity that small. See the viability report (findings R18/R19) for the exact mechanism.
