...
- **A Monitor tick that decides *not* to scale is Debug-log-only, never a metric/trace event.** `scale.*`/`"RingBufferPlus.Scale"` only exist for a dispatched scale operation, so a tick that leaves capacity unchanged (target already equals `CurrentCapacity`, or the change is smaller than the effective `MonitorDeadband`) is otherwise invisible. Enable `LogLevel.Debug` on the configured `Logger` to see `"Monitor tick: demand=..., target=..., current=..."` on every tick once at least 2 samples exist, including the no-scale cases - this is the only way to answer "why didn't it scale" from telemetry, short of reproducing against `AutoScaleMonitor.EvaluateTarget` directly.
