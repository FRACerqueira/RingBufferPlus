...
- **`HeartBeat` acquires count too.** Every `PulseHeartBeat` tick calls `AcquireAsync` internally to fetch the item your callback inspects, so it shows up in `acquire.duration`/`acquire.faults` and produces its own `"RingBufferPlus.Acquire"` span, indistinguishable from a caller-initiated acquire by tag. If your dashboard cares about the difference, there is currently no tag for it — correlate by volume/interval against your configured `PulseHeartBeat` instead.
