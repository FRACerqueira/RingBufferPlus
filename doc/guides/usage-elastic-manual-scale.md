...
`SwitchToAsync` posts a command to the engine's single-consumer loop. By default it returns `true` as soon as the move is *scheduled*, without waiting for it to finish — but "scheduled" itself is only immediate when the engine is idle; behind an already in-flight scale operation, this call queues and can take as long as that operation's own deadline before it returns at all (a scale-up's deadline is `quantity × FactoryTimeout`, not `baseTimer`/`numberSamples` — those only configure the scale-down sampling cadence used by autoscale-on-fault; see the [lock guide](usage-lock-when-scaling.md) for the full detail). Enable `LockWhenScaling()` if you need the returned `bool` to reflect whether the target capacity was actually, fully reached. Either way, asking to switch to the capacity the buffer is already at returns `false` immediately, with nothing scheduled — and `AcquireAsync` is never blocked by an in-progress scale, regardless of `LockWhenScaling`.
