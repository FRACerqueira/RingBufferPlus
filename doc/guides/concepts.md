...
flowchart LR
    ...
    G --> E
    C -.->|"manual: SwitchToAsync<br/>auto: acquire faults / sample median"| H["Scale up / Scale down"]
    H -.-> C
...
1. **Configure** — start from `RingBuffer<T>.New(name)`, set `Factory` (required) and any of `Logger`, `HeartBeat`, `AcquireTimeout`, `OnError`, `BackgroundLogger`, then commit to a mode with `FixedCapacity(n)` or `ElasticCapacity(init, min, max, ...)`. In the generated API reference, these shared operations look duplicated - `Factory`, `HeartBeat`, `AcquireTimeout`, `OnError` are each declared on all 4 builder interfaces (`IRingBufferBuilder<T>`, `IRingBufferFixedBuilder<T>`, `IRingBufferElasticBuilder<T>`, `IRingBufferAutoScaleBuilder<T>`) instead of once on a shared base. That is deliberate, not an oversight: each interface's own version returns *that same interface type*, so the fluent chain stays within the mode you already committed to (fixed vs. elastic vs. autoscale) instead of widening back out to a less specific type - see [ADR007](../adr/ADR007V02-redesign-of-the-public-fluent-api-surface.md) for why the chain is split into mode-specific types in the first place.
...
4. **Scale** (elastic only) — either you call `SwitchToAsync(ScaleSwitch.MinCapacity | InitCapacity | MaxCapacity)` yourself, or, with `AutoScaleAcquireFault(n)`, the buffer scales up on its own after `n` acquire timeouts and scales back down on its own once sampled usage supports it. The two are mutually exclusive at compile time (see [ADR007](../adr/ADR007V02-redesign-of-the-public-fluent-api-surface.md)) — a builder that calls `AutoScaleAcquireFault` never returns a type exposing `SwitchToAsync`.
5. **Dispose** — `await using`/`DisposeAsync()` on the service itself stops all background activity (heartbeat, sampling, logging) and disposes every item still in the pool. This is the only disposal contract — there is no synchronous `Dispose()` ([ADR005](../adr/ADR005V01-async-disposal-strategy-and-graceful-shutdown.md)). If a `HeartBeat` callback is still orphaned from a previous timeout when this runs, `DisposeAsync()` can take up to an extra `pulse` interval waiting for its deferred resource disposal to finish - see the [heartbeat guide](usage-heartbeat.md) if you're budgeting a shutdown grace period around this.
