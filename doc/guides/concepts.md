...
These three numbers only exist for an elastic buffer ([`ElasticCapacity`](usage-elastic-autoscale.md)). A fixed buffer ([`FixedCapacity`](usage-fixed-capacity.md)) has exactly one capacity value and never scales.
