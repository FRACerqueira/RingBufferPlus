...
4. **Scale** (elastic only) — either you call `SwitchToAsync(ScaleSwitch.MinCapacity | InitCapacity | MaxCapacity)` yourself, or, with `AutoScaleAcquireFault(n)`, the buffer scales up on its own after `n` acquire timeouts and scales back down on its own once sampled usage supports it. The two are mutually exclusive at compile time (see [ADR007](../adr/ADR007V01-redesign-of-the-public-fluent-api-surface.md)) — a builder that calls `AutoScaleAcquireFault` never returns a type exposing `SwitchToAsync`.
5. **Dispose** — `await using`/`DisposeAsync()` on the service itself stops all background activity (heartbeat, sampling, logging) and disposes every item still in the pool. This is the only disposal contract — there is no synchronous `Dispose()` ([ADR005](../adr/ADR005V01-async-disposal-strategy-and-graceful-shutdown.md)).
