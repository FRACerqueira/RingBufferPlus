...
**Revised again on 2026-08-11 — migration communication scope:** the maintainer decided against producing a dedicated `doc/guides/migration/v4-to-v5.md` document. This is **not** derived from the [ADR006](./ADR006V01-mandate-for-a-complete-product-overhaul-in-v5-with-authorized-breaking-changes.md) mandate — "no commitment to the current version" answers whether v5.0.0 may break compatibility, not whether existing consumers are told how to move past the break; ADR006 itself lists "existing consumers still need clear notice and a migration path" as a decision driver and "requires a robust v4→v5 migration guide" as a named negative consequence. This is a separate, deliberate scope/effort decision: `CHANGELOG.md`'s `[Unreleased]`/"Breaking changes v5.0.0" section is the **sole** migration reference — no separate guide, no per-scenario checklist, no before/after walkthroughs beyond what the CHANGELOG entries themselves state. The "Mandatory communication" item below and [ADR006](./ADR006V01-mandate-for-a-complete-product-overhaul-in-v5-with-authorized-breaking-changes.md)'s negative consequences are updated accordingly.

**Amended on 2026-08-20 (v5.0.0 hardening, before wide release) — Technical Story:** the v5.0.0 product-viability audit (`TODO/relatorio-viabilidade-ringbufferplus-v5.md`) produced a batch of fixes for the next release: the large majority are internal bug fixes and additive/behavior-refining changes with no public signature change (the exception-boundary fix, dispose-race fixes, heartbeat timeout enforcement, unconditional disposal cleanup, the scale-up partial-capacity behavior of [ADR010](./ADR010V01-allow-partial-capacity-gains-when-a-scale-up-cannot-fully-complete.md), the autoscale target-formula fix, a new `success` metric tag) — squarely minor/patch material under Stage (b)'s strict SemVer. One item in the same batch is a genuine breaking change: [ADR007 V02](./ADR007V02-redesign-of-the-public-fluent-api-surface.md) removes the public no-op `LockWhenScaling(bool)` from `IRingBufferAutoScaleBuilder<T>` without the `[Obsolete]` cycle Stage (b) otherwise requires (justified there on its own terms — the symbol never carried real behavior for the cycle to protect). Under strict SemVer, a public removal requires a MAJOR bump (6.0.0), which would force this one narrow removal to carry an otherwise minor/patch-shaped release into a major version bump.

**Decision:** the next release is versioned **5.1.0**, not 6.0.0, as a second, explicit, narrow exception to this ADR's Stage (b) — this time to the *version-number* rule specifically, not the deprecation-cycle rule (already excepted separately in [ADR007 V02](./ADR007V02-redesign-of-the-public-fluent-api-surface.md)). The reasoning mirrors that exception: `LockWhenScaling` on `IRingBufferAutoScaleBuilder<T>` never represented real, relied-upon behavior — its only documented contract was "does nothing" — so its removal does not carry the compatibility weight a MAJOR bump exists to signal, even though it is, mechanically, a public API removal. Labeling the whole release 6.0.0 over one such symbol would overstate its impact and obscure the release's real shape (predominantly bug fixes and behavior refinements) behind a major-version signal that suggests far more disruption than is actually present.
...
* *(2026-08-20 amendment)* A consumer who reads only the version number (5.1.0) and assumes "no breaking changes" without reading the CHANGELOG will be surprised by the `LockWhenScaling` removal — mitigated by the same `CHANGELOG.md`/release-notes discipline this ADR already relies on for stage (a), not by the version number itself.
* *(2026-08-20 amendment)* A second version-numbering exception this soon after the deprecation-cycle exception ([ADR007 V02](./ADR007V02-redesign-of-the-public-fluent-api-surface.md)) makes it more important than ever that neither is treated as routine — both are recorded here explicitly as narrow, non-repeating, so a third such request should be met with real scrutiny, not assumed approval by precedent.
...
* Refined by: [ADR006V01](./ADR006V01-mandate-for-a-complete-product-overhaul-in-v5-with-authorized-breaking-changes.md) — mandate that authorized stage (a)'s v5.0.0 reset.
* Refined by: [ADR006V02](./ADR006V02-mandate-for-a-complete-product-overhaul-in-v5-with-authorized-breaking-changes.md) — mandate that authorizes the second reset for v6.0.0 and moves stage (b)'s resumption point.
