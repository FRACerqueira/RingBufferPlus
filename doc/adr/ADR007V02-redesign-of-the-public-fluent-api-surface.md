...
**Amended on 2026-08-20 (v5.0.0 hardening, before wide release) — Technical Story:** the v5.0.0 product-viability audit (`TODO/relatorio-viabilidade-ringbufferplus-v5.md`, findings U-06/U-10) found that `LockWhenScaling` had in fact been retained on `IRingBufferAutoScaleBuilder<T>` as a documented no-op ("accepted but has no observable effect... retained here only so the fluent chain compiles unchanged after calling `AutoScaleAcquireFault(byte)`") — exactly the kind of silent trap the "concrete design" above set out to eliminate for the `AutoScaleAcquireFault`/`SwitchToAsync` exclusivity, just left standing for this one method. It had already misled a real caller: this repo's own `samples/RingBufferPlusRabbitSample/Program.cs` chained `.LockWhenScaling()` immediately before `.AutoScaleAcquireFault()`, believing it changed locking behavior for that buffer — it did not, and that sample scenario was removed in the same audit (finding U-06) for exactly this reason.

**Amendment:** `LockWhenScaling(bool value = true)` is removed entirely from `IRingBufferAutoScaleBuilder<T>`. Any call site chaining it after `.AutoScaleAcquireFault()` now fails to compile instead of silently compiling and doing nothing — completing, for this specific spot, the compile-time-safety goal this ADR's original "concrete design" already established for the `AutoScaleAcquireFault`/`SwitchToAsync` exclusivity.

**Policy note:** this amendment is made **without** the `[Obsolete]` deprecation cycle that `CONTRIBUTING.md` and [ADR004](./ADR004V01-semantic-versioning-policy-and-fluent-api-stability.md) otherwise require before removing a public symbol, as an explicit, narrow exception. The deprecation policy exists to protect callers legitimately relying on a symbol's *documented behavior*; this symbol's entire documented behavior was already "has no observable effect," so there is no real behavior for a deprecation window to protect reliance on — only the trap itself, which a full extra release cycle of `[Obsolete]`-but-still-compiling would have kept alive rather than closed.
...
* *(2026-08-20 amendment)* Closes the one spot where the original design left a documented no-op standing instead of a compile error, and removes a proven source of real confusion (the RabbitMQ sample).
...
* *(2026-08-20 amendment)* A caller who wrote `.ElasticCapacity(...).LockWhenScaling().AutoScaleAcquireFault()` gets a compile error on upgrading, rather than an `[Obsolete]` warning first — an explicit, documented exception to the deprecation policy, not a silent one.
...
* Related: [ADR004](./ADR004V01-semantic-versioning-policy-and-fluent-api-stability.md) — this surface change is one of those covered by stage (a) — the single v5.0.0 reset; the 2026-08-20 amendment is the deprecation-policy exception this same ADR004 requires to be justified explicitly.
* Related: `TODO/relatorio-viabilidade-ringbufferplus-v5.md`, findings U-06 and U-10 — the audit that surfaced the 2026-08-20 amendment, including the real-world confusion in `samples/RingBufferPlusRabbitSample`.
