...
* **`OnError` simplified**: to `Action<Exception>` (the logger is already configured separately), called inline, no queue.
...
* Related: [ADR001V03](./ADR001V03-concurrency-model-for-ring-buffer-manager-scale-up-and-down.md) — the always-on floor guard/backlog-reactive/Monitor signals the pin coexists with, and their fixed priority over it.
