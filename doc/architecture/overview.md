...
- **`RingBufferManager<T>`** (`src/RingBufferPlus/Core/RingBufferManager.cs`) is the only concrete implementation of `IRingBufferService<T>`/`IRingBufferManualScaleService<T>`. It owns the engine command channel and the available-items channel, plus background tasks for Creator (scale-up) and Removal (scale-down) dispatch, with a single consumer loop that is the sole writer of scale state. See [ADR001](../adr/ADR001V03-concurrency-model-for-ring-buffer-manager-scale-up-and-down.md).
